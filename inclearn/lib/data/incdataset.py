...
class IncrementalDataset:
    ...
    def new_task(self, memory=None, memory_val=None):
        ...
        test_loader = self._get_loader(x_test, y_test, np.zeros((x_test.shape[0],)), mode="test", shuffle=False)
    ...
    def _setup_data(
        ...
        for i in range(len(self.increments)):
            for j in range(sum(self.increments[:i]), sum(self.increments[:i+1])):
                self._map_to_task[j] = i
