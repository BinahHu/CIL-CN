...
class MetricLogger:
    ...
    def __init__(self, nb_tasks, nb_classes, increments, is_task):
        self.metrics = collections.defaultdict(list)

        self.nb_tasks = nb_tasks
        self.is_task = is_task
        if is_task:
            self.nb_classes = self.nb_tasks
            self.increments = [1 for _ in range(len(increments))]
        else:
    ...
    def log_task(self, ypreds, ytrue, task_size, zeroshot=False):
        self.metrics["accuracy"].append(
            accuracy_per_task(ypreds, ytrue, task_size=10, topk=1, is_task=self.is_task)
        )  # FIXME various task size
        self.metrics["accuracy_top5"].append(
            accuracy_per_task(ypreds, ytrue, task_size=None, topk=5, is_task=self.is_task)
        )
        self.metrics["accuracy_per_class"].append(
            accuracy_per_task(ypreds, ytrue, task_size=1, topk=1, is_task=self.is_task)
        )
...
def accuracy_per_task(ypreds, ytrue, task_size=10, topk=1, is_task=False):
